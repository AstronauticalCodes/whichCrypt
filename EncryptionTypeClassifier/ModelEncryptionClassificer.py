...
def extract_features(hash_str):
    features = {
        ...
        'sym_count': sum(c in symbols for c in hash_str )
    }
    return features

data['features'] = data.get('Encrypted-Text').apply(extract_features)
...
#
while True:
    hash_input = input("ENTER CIPHERTEXT : ")
